...
int main(){
	...
	queue<Coordinate> nearest_elements;
	...
	for(int i = 0; i < k_points; i++){
		nearest_elements.push(coordinates[i]);
	}

	//Sort the queue
	while(!nearest_elements.empty()){
		cout << nearest_elements.front().x << " , " << nearest_elements.front().y << endl;
